...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const submenuWrapper = nivel2.querySelector('.submenu-wrapper');
  const wordsWrapper = nivel2.querySelector('.words-wrapper');
  const displayContainer = nivel2.querySelector('.words-display');
  const btnBackToMenu = nivel2.querySelector('.btn-back-to-menu');

  // 1. Capturar clics en los botones del menú de subcategorías
  nivel2.addEventListener('click', (event) => {
  ...
  nivel2.addEventListener('restoreSection', (event) => {
