...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const submenuWrapper = nivel1.querySelector('.submenu-wrapper');
  const wordsWrapper = nivel1.querySelector('.words-wrapper');
  const displayContainer = nivel1.querySelector('.words-display');
  const btnBackToMenu = nivel1.querySelector('.btn-back-to-menu');

  // 1. Capturar clics en los botones del menú de subcategorías
  nivel1.addEventListener('click', (event) => {
  ...
  nivel1.addEventListener('restoreSection', (event) => {
